...
!-------------------------------------------------------------------------------
!> @brief Interface to temperature on pressure levels code.

module temp_on_pres_kernel_mod
  ...
  type, public, extends(kernel_type) :: temp_on_pres_kernel_type
    private
...
!        arg_type(GH_SCALAR_ARRAY,GH_REAL, GH_READ, 1), see PSyclone issue #1312
  ...
  contains
    procedure, nopass :: temp_on_pres_code
  end type temp_on_pres_kernel_type

  public :: temp_on_pres_code

contains

  !> @details Calculates temperature on pressure levels, with appropriate
  ...
  subroutine temp_on_pres_code(nlayers,                         &
                               data_in,                         &
                               ex_at_data,                      &
                               height_wth,                      &
                               nplev,                           &
    ...
    real(kind=r_def), parameter:: extrap_height = 200.0_r_def
    ...
    do kp = 1, nplev
      ...
      if (level_above == -1_i_def) then
        ! Desired level is above model top, isothermal extrapolate up
        ...
        data_out(map_out(1)+kp-1) = data_in(map_in(1)+level_extrap) * &
                    (desired_ex/ex_at_data(map_in(1)+level_extrap))**ex_power
      else
  ...
  end subroutine temp_on_pres_code

end module temp_on_pres_kernel_mod
