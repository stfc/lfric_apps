...
module easyaerosol_scatdiag_kernel_mod
...
type, public, extends(kernel_type) :: easyaerosol_scatdiag_kernel_type
  private
...
contains
  procedure, nopass :: easyaerosol_scatdiag_code
end type

public :: easyaerosol_scatdiag_code

contains

!> @brief         Derive the scattering coefficient for the easyaerosol diagnostic
!>                based on the extinction minus absorption from the climatology.
!>                This is not a totally straightforward operation as the diag arrays
!>                have a different shape to those containing the easyaerosol
!>                climatology.
!>
!> @param[in]     nlayers          The number of layers
!> @param[in]     n_band           Number of spectral bands (sw or lw)
!> @param[in]     easy_ext         Aerosol extinction from easyaerosol
!> @param[in]     easy_abs         Aerosol absorption from easyaerosol
!> @param[in,out] easy_scat_diag   Aerosol property diagnostic
...
subroutine easyaerosol_scatdiag_code( nlayers, n_band,                         &
                                      easy_ext, easy_abs,                      &
                                      easy_scat_diag,                          &
  ...
  real(kind=r_def), intent(in),    dimension(undf_easy) :: easy_ext
  real(kind=r_def), intent(in),    dimension(undf_easy) :: easy_abs
  real(kind=r_def), intent(inout), dimension(undf_diag) :: easy_scat_diag
  ...
  do i_band = 1, n_band
    ...
    easy_scat_diag (i_diag) = 0.0_r_def
    ...
    do k = 1, nlayers
      ...
      easy_scat_diag(i_diag) =  easy_ext(i_easy) - easy_abs(i_easy)

    end do
  end do

end subroutine easyaerosol_scatdiag_code

end module easyaerosol_scatdiag_kernel_mod
